    ...
    public sealed class AttachmentExtractor : IAttachmentExtractor, IDisposable
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            using (await _semaphoreLocks.LockAsync(outputFolder, cancellationToken).ConfigureAwait(false))
            {
                ...
                foreach (var attachment in mediaSource.MediaAttachments)
                {
                    ...
                    targets.Add(new AttachmentTarget(attachment.Index, attachmentPath.EscapeProcessArgument()));
            ...
            CancellationToken cancellationToken)
        {
            ...
            var result = await _ffRunner.RunAsync(
                ...
                {
                    Input = inputPath,
                    Targets = [new AttachmentTarget(attachmentStreamIndex, outputPath.EscapeProcessArgument())]
                },
                cancellationToken).ConfigureAwait(false);
