...
public sealed record LoudnessRequest : FFRequest
{
    ...
    public override void BuildArguments(StringBuilder builder)
    {
        ...
        // summary, but the per-frame lines are what let a truncated or stalled measurement be distinguished
        // summary, but the per-frame lines are what let a truncated or stalled measurement be distinguished
        // from a complete one. The caller's regex will match the summary block, but not any incomplete output.
        //
        // -f null discards the decoded audio; the analysis is the entire point of the run.
        builder.Append("-i \"")
            .Append(Path.EscapeProcessArgument())
